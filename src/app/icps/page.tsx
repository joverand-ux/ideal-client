  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Ideal Client Profiles</h1>
          <p className="text-gray-500 text-sm mt-1">Define the types of companies you want to reach.</p>
        </div>
        {profileId && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {icps.map((icp) => (
          <div key={icp.id} className="bg-white border border-gray-200 rounded-xl p-5 hover:border-indigo-300 hover:shadow-sm transition-all cursor-pointer" onClick={() => openEdit(icp)}>
            <div className="flex items-start justify-between mb-3">
              <h3 className="font-semibold text-gray-900">{icp.name}</h3>
              <button onClick={(e) => { e.stopPropagation(); del(icp.id); }} className="text-gray-300 hover:text-red-500 transition-colors">
                <X size={14} />
              </button>
            </div>
            {icp.industries.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2">
                {icp.industries.slice(0, 3).map((ind) => (
                  <span key={ind} className="bg-blue-50 text-blue-700 text-xs px-2 py-0.5 rounded-full">{ind}</span>
                ))}
                {icp.industries.length > 3 && <span className="text-gray-400 text-xs">+{icp.industries.length - 3}</span>}
              </div>
            )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-gray-200 rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-lg">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-900">{editing ? "Edit ICP" : "Create ICP"}</h2>
            ...
            <div className="flex gap-3 p-5 border-t border-gray-100">
              <button onClick={save} disabled={saving || !form.name}
                className="flex-1 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2">
                {saving && <Spinner size={14} />}{saving ? "Saving…" : "Save"}
              </button>
              <button onClick={closeModal} className="px-4 py-2 rounded-lg text-sm text-gray-500 hover:text-gray-900 border border-gray-200 hover:border-gray-300 transition-colors">
                Cancel
              </button>
            </div>
          </div>
